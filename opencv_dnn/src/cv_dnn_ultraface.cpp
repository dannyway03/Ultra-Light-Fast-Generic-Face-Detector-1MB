...
UltraFace::UltraFace(const std::string model_path, int input_width,
...
int
UltraFace::detect(cv::Mat &img, std::vector<FaceInfo> &face_list)
{
  if(img.empty())
    {
  ...
  cv::Mat inputBlob = cv::dnn::blobFromImage(
      img, 1.0 / 128, cv::Size(in_w, in_h), cv::Scalar(127, 127, 127), true);
  ...
  generateBBox(bbox_collection, out_blobs[0], out_blobs[1], score_threshold,
               num_anchors);
...
void
UltraFace::generateBBox(std::vector<FaceInfo> &bbox_collection, cv::Mat scores,
  ...
  for(int i = 0; i < num_anchors; i++)
    {
      ...
      if(score_value[2 * i + 1] > score_threshold)
        {
          FaceInfo rects = { 0 };
          float x_center = bbox_value[i * 4] * center_variance * priors[i][2]
                           + priors[i][0];
          float y_center
              = bbox_value[i * 4 + 1] * center_variance * priors[i][3]
                + priors[i][1];
...
void
UltraFace::nms(std::vector<FaceInfo> &input, std::vector<FaceInfo> &output,
               int type)
{
  std::sort(
